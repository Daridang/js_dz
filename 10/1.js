...
function createUserCard(userData) {
  ...
  dob.textContent = `DOB: ${new Date(userData.dob.date).toLocaleDateString()} (Age: ${userData.dob.age})`;
  card.appendChild(dob);

  const registered = document.createElement('p');
  registered.textContent = `Registered: ${new Date(userData.registered.date).toLocaleDateString()} (Account Age: ${userData.registered.age} years)`;
